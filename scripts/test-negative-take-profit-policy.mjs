...
assert.match(BINANCE_NEGATIVE_TP_TO_ENTRY_VERSION, /CAP_TSL_EXCLUDED_ROE20/);
...
assert.equal(shouldMoveNegativeTpToEntry({ roe: -20, capTsl: true }), false);
assert.equal(shouldMoveNegativeTpToEntry({ roe: -80, capTsl: true }), false);
...
assert.match(serverSource, /if \(isCapTslSymbol\(symbol\)\) \{[\s\S]*negativeSince\.delete\(symbol\);[\s\S]*return;/);
assert.match(serverSource, /function negativeEntryGuardHasPriority[\s\S]*if \(isCapTslSymbol\(symbol\)\) return false;/);
...
assert.match(ordersHtml, /không dời TP về entry khi ROE ≤ -20%/);
assert.match(ordersJs, /không dời TP về entry khi ROE ≤ -20%/);

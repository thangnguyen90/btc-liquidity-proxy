    ...
    return `
      <tr data-sym="${sym}">
        ...
        <td style="text-align:center">
          <input type="checkbox" class="tsl-exclude-cb" data-sym="${sym}" title="Chỉ giữ ROE ≥10% → SL +1%; không nâng tiếp ở 15%/20%+. TP về entry khi ROE ≤ -20% vẫn chạy."
...
async function loadTsl() {
  try {
    ...
    tslStatus.textContent = data.enabled
      ? `ProfitLock · Manual/V2: 10→1, 15→5, 20→10 · Cap TSL checked: 10→1 tối đa; TP→entry khi âm ≤20% vẫn chạy · TSL cũ OFF`
